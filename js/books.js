...
const getProducts = async () => {
  ...
  const fictionCenter = document.querySelector(".fiction-center");
  const schoolCenter = document.querySelector(".school-center");
  const preparationCenter = document.querySelector(".preparation-center");
  const literatureCenter = document.querySelector(".literature-center");
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    const fictionProducts = await filterArray("fiction");
    
    const schoolProducts = await filterArray("school");
    const preparationProducts = await filterArray("preparation");
    const literatureProducts = await filterArray("literature");
    displayProducts(defaultProducts, productCenter);
    displayProducts(specialProducts, specialCenter);
    displayProducts(fictionProducts, fictionCenter);
    
    displayProducts(schoolProducts, schoolCenter);
    displayProducts(preparationProducts, preparationCenter);
    displayProducts(literatureProducts, literatureCenter);
